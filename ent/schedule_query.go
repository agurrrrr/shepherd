...
func (_q *ScheduleQuery) QueryProject() *ProjectQuery {
...
func (_q *ScheduleQuery) WithProject(opts ...func(*ProjectQuery)) *ScheduleQuery {
	query := (&ProjectClient{config: _q.config}).Query()
...
func (_q *ScheduleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Schedule, error) {
	...
	if query := _q.withProject; query != nil {
		if err := _q.loadProject(ctx, query, nodes, nil,
			func(n *Schedule, e *Project) { n.Edges.Project = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ScheduleQuery) loadProject(ctx context.Context, query *ProjectQuery, nodes []*Schedule, init func(*Schedule), assign func(*Schedule, *Project)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Schedule)
	for i := range nodes {
		if nodes[i].project_schedules == nil {
			continue
		}
		fk := *nodes[i].project_schedules
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "project_schedules" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
