...
func (_q *ProjectQuery) QuerySheep() *SheepQuery {
...
func (_q *ProjectQuery) QuerySchedules() *ScheduleQuery {
...
// QueryWikiVersions chains the current query on the "wiki_versions" edge.
func (_q *ProjectQuery) QueryWikiVersions() *WikiPageVersionQuery {
...
func (_q *ProjectQuery) WithSheep(opts ...func(*SheepQuery)) *ProjectQuery {
	query := (&SheepClient{config: _q.config}).Query()
...
func (_q *ProjectQuery) WithSchedules(opts ...func(*ScheduleQuery)) *ProjectQuery {
	query := (&ScheduleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithWikiVersions tells the query-builder to eager-load the nodes that are connected to
// the "wiki_versions" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ProjectQuery) WithWikiVersions(opts ...func(*WikiPageVersionQuery)) *ProjectQuery {
	query := (&WikiPageVersionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := _q.withSheep; query != nil {
		if err := _q.loadSheep(ctx, query, nodes, nil,
			func(n *Project, e *Sheep) { n.Edges.Sheep = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withSchedules; query != nil {
		if err := _q.loadSchedules(ctx, query, nodes,
			func(n *Project) { n.Edges.Schedules = []*Schedule{} },
			func(n *Project, e *Schedule) { n.Edges.Schedules = append(n.Edges.Schedules, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withWikiVersions; query != nil {
...
func (_q *ProjectQuery) loadSheep(ctx context.Context, query *SheepQuery, nodes []*Project, init func(*Project), assign func(*Project, *Sheep)) error {
	...
	query.withFKs = true
	query.Where(predicate.Sheep(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.SheepColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.project_sheep
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_sheep" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_sheep" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *ProjectQuery) loadSchedules(ctx context.Context, query *ScheduleQuery, nodes []*Project, init func(*Project), assign func(*Project, *Schedule)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Schedule(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.SchedulesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.project_schedules
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_schedules" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_schedules" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *ProjectQuery) loadSkills(ctx context.Context, query *SkillQuery, nodes []*Project, init func(*Project), assign func(*Project, *Skill)) error {
	...
	for _, n := range neighbors {
		fk := n.project_skills
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_skills" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (_q *ProjectQuery) loadWikiVersions(ctx context.Context, query *WikiPageVersionQuery, nodes []*Project, init func(*Project), assign func(*Project, *WikiPageVersion)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.WikiPageVersion(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.WikiVersionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.project_wiki_versions
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_wiki_versions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
