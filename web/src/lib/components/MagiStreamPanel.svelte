<script>
	...
	// Parse a raw line into { slot, text } where slot is 0, 1, 2, or '*'.
	function parseLine(raw) {
		...
		// No prefix → unified panel (backward compat)
		return { slot: '*', text: line };
	}

	// Classify and route lines to panels
	let panelData = $derived.by(() => {
		const proposers = [[], [], []]; // slot 0, 1, 2
		const unified = [];
		// Track completion status per slot
		...
		for (const raw of lines) {
			...
			if (slot === '*') {
				...
				if (s >= 0 && s <= 2) {
					...
					proposers[s].push(text);
				}
			}
		}

		return { proposers, unified, completed };
	});
	...
	// Trigger markdown rendering for unified panel text blocks
	$effect(() => {
		for (const t of panelData.unified) {
			if (t && t.trim()) {
				ensureRendered(t);
			}
		}
	...
	// Auto-scroll proposer panels
	let propContainers = $state([null, null, null]);
	let unifiedContainer = $state(null);

	$effect(() => {
		// Scroll each proposer panel to bottom
		for (let i = 0; i < 3; i++) {
			const el = propContainers[i];
			if (el && panelData.proposers[i].length > 0) {
				requestAnimationFrame(() => {
					el.scrollTop = el.scrollHeight;
				});
			}
		}
		// Scroll unified panel
		if (unifiedContainer && panelData.unified.length > 0) {
			requestAnimationFrame(() => {
	...
	// Check if any proposer has received output yet
	let hasProposerOutput = $derived(
		panelData.proposers.some((p) => p.length > 0)
	);
</script>
...
<div class="magi-panel">
			...
			{#each personaInfo as persona, i}
				<div class="proposer-panel" style="--persona-color: {persona.color}">
					<div class="proposer-header">
						...
						{:else if panelData.proposers[i].length > 0}
							<span class="proposer-status streaming">●</span>
						{/if}
					</div>
					<div class="proposer-content" bind:this={propContainers[i]}>
						{#if panelData.proposers[i].length === 0}
							<span class="proposer-idle">대기 중...</span>
						{:else}
							{#each panelData.proposers[i] as text, j (j)}
								<div class="proposer-line">
									{#if text.includes('응답 완료') || text.includes('응답 실패')}
										<div class="proposer-summary" class:fail="{text.includes('응답 실패')}">
											{text.trim()}
										</div>
									{:else}
										<span class="proposer-token">{text}</span>
									{/if}
								</div>
							{/each}
						{/if}
	...
	<div class="unified-panel">
		...
		<div class="unified-content" bind:this={unifiedContainer} style="max-height: {maxHeight}">
			{#if panelData.unified.length === 0 && !hasProposerOutput}
				<span class="empty-output">No output yet</span>
			{:else}
				{#each panelData.unified as text, i (i)}
