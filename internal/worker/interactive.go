...
// mcpGuideText is a concise behavioral note for shepherd's MCP tools. The full
// tool list and parameter schemas are already exposed to the agent through MCP
// registration, so re-enumerating every tool (and its parameters) in the system
// prompt is redundant. We keep only the non-obvious usage guidance here.
const mcpGuideText = `[Shepherd MCP — usage notes]
...
func getProjectWikiContext(sheepName, prompt string) string {
	...
	sb.WriteString(fmt.Sprintf("- Replace a section:  wiki_edit (project_name=%q, slug, mode=\"section\", section=\"<heading text>\", line_text) — replaces the section body in place; empty line_text clears it\n", projectName))
	sb.WriteString(fmt.Sprintf("- Find & replace:     wiki_edit (project_name=%q, slug, mode=\"find_replace\", find, replace) — find is a whole-page regex (multiline OK); empty replace deletes the match\n", projectName))
