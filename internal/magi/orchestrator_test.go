...
func (d *dualFake) epCall(ctx context.Context, ep EndpointRef, systemPrompt, userPrompt string, temperature float32, maxTokens int) (string, embedded.ChatUsage, error) {
	d.mu.Lock()
	d.epTotal++
	idx := d.epCount[ep.ID]
	d.epCount[ep.ID]++
	...
	return funcs[idx](ctx, ep, systemPrompt, userPrompt, temperature, maxTokens)
}
